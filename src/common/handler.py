...
def _load_fast_grib_loader():
    try:
        ...
        from common.grib_loader import load_grib_fast

        return load_grib_fast


def extract_timestamp(filepath, use_timezone_utc=False, round_to_minute=False, isoformat=False):
    """Compact timestamp extractor for MRMS and GOES-style filenames."""
...
class FileHandler:
    ...
    def subset_dataset(self, ds, lat_limits, lon_limits):
        try:
            ...
            self.io.write_debug(
                f"Subset dataset to lat: {lat_limits}, lon: {lon_limits} (adjusted to {l_min:.2f}, {l_max:.2f})"
            )
            return ds
        except Exception as e:

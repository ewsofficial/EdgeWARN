...
def pipeline(log_queue, dt):
    ...
    try:
        ...
        detect.main(filepath_old, filepath_new, ps_old, ps_new, pt_old, pt_new, lat_limits, lon_limits, Path("stormcell_test.json"), render_files=True)
        integration.main(render_files=True)
...
if __name__ == "__main__":
    if args.nogui:
        ...
        try:
            print(f"Running EdgeWARN v0.6.3-alpha")
            print(f"Latitude limits: {lat_limits}, Longitude limits: {lon_limits}")
            main(ui_process)
        except KeyboardInterrupt:
            print("CTRL+C detected, exiting ...")

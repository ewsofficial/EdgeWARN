...
class StormCellDetector:
    ...
    def propagate_cells(reflectivity, lat_grid, lon_grid, seed_dbz=50,
        ...
        # Find Timestamps
        if filepath:
            scan_time = timestamp.extract_timestamp_from_filename(filepath)
        else:
...
# Preserve original function names for backward compatibility
def bbox_to_points(bbox):
    return StormCellDetector.bbox_to_points(bbox)

def polygon_to_bbox(polygon):
    return StormCellDetector.polygon_to_bbox(polygon)

def convert_lon_0_360_to_pm180(lon):
    return StormCellDetector.convert_lon_0_360_to_pm180(lon)

def get_alpha_shape_from_mask(mask, lat_grid, lon_grid, alpha=0.1):
    return StormCellDetector.get_alpha_shape_from_mask(mask, lat_grid, lon_grid, alpha)

def propagate_cells(reflectivity, lat_grid, lon_grid, seed_dbz=50,
                    expand_dbz=40, min_gates=25, max_iterations=100, alpha=0.1, filepath=None):
    return StormCellDetector.propagate_cells(reflectivity, lat_grid, lon_grid, seed_dbz,
                                            expand_dbz, min_gates, max_iterations, alpha, filepath)

def merge_connected_small_cells(cells, size_ratio_threshold=0.9, buffer_km=1.0, alpha=0.1):
    return StormCellDetector.merge_connected_small_cells(cells, size_ratio_threshold, buffer_km, alpha)
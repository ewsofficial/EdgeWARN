...
class MesocycloneModule(AnalysisModule):
    ...
    def _score_strength(self, low: Optional[Dict[str, Any]], mid: Optional[Dict[str, Any]]) -> float:
        ...
        low_peak = float(low.get("peak_value", 0.0))
        ...
        mid_peak = float(mid.get("peak_value", 0.0))
    ...
    def _score_morphology(self, low: Optional[Dict[str, Any]], mid: Optional[Dict[str, Any]], storm_morphology: Dict[str, Any]) -> float:
        ...
        widths = [float(item.get("width_km", 0.0)) for item in candidates]
        ellipticities = [float(item.get("ellipticity", 0.0)) for item in candidates]
        aspect_ratios = [float(item.get("aspect_ratio", 1.0)) for item in candidates]
        ...
        reflectivity_aspect = float(storm_morphology.get("aspect_ratio", 1.0))
        reflectivity_linearity = float(storm_morphology.get("linearity", 0.0))
    ...
    def _persistence_bonus(self, storm_entry: Dict[str, Any], history_cache: Optional[Any]) -> float:
        ...
        for entry in history:
            ...
            low = azshear.get("low")
            if low and alignment.get("is_vertically_aligned"):
                qualifying += 1
                continue

            prior_result = entry.get("modules", {}).get(self.name, {})
            if prior_result.get("classification") in {"mesocyclone", "deep_mesocyclone"}:
                qualifying += 1
    ...
    def run(self, storm_entry: Dict[str, Any], environment: Optional[Dict[str, Any]] = None, history_cache: Optional[Any] = None) -> None:
        ...
        if low and low.get("peak_value", 0.0) >= cfg.LOW_PEAK_STRONG:
            triggers.append("LOW_LEVEL_ROTATION")
        if mid and mid.get("peak_value", 0.0) >= cfg.MID_PEAK_STRONG:
            triggers.append("MID_LEVEL_ROTATION")
        if alignment.get("is_vertically_aligned"):
            triggers.append("VERTICAL_ALIGNMENT")
        if low and low.get("ellipticity", 0.0) >= cfg.MIN_ELLIPTICITY:
            triggers.append("ELLIPTICAL_SIGNATURE")
        if low and low.get("width_km", cfg.MAX_WIDTH_KM + 1.0) <= cfg.MAX_WIDTH_KM:
        ...
        classification = "none"
        if confidence >= cfg.DEEP_CONFIDENCE and alignment.get("is_vertically_aligned") and mid:
            classification = "deep_mesocyclone"
        elif confidence >= cfg.MIN_CLASSIFY_CONFIDENCE and low:
        ...
        storm_entry["modules"][self.name] = {
            ...
            "diagnostics": {
                ...
                "aligned": bool(alignment.get("is_vertically_aligned", False)),
                "low_peak": None if not low else low.get("peak_value"),
                "mid_peak": None if not mid else mid.get("peak_value"),

...
export function createCompatibilityRouter({ analysis, renders, ancillary, packageVersion, serviceRegistry }) {
  ...
  router.get('/renders/get-items', async (req, res, next) => { try { deprecate(res); res.json((await renders.listProducts()).map((item) => item.legacyId)); } catch (e) { next(e); } });
  router.get('/renders/fetch', async (req, res, next) => { try { deprecate(res); const product = getProductByLegacyId(value(req.query.product)); if (!product) return res.status(404).json({ error: 'Unknown product or no mapping found' }); res.json(await renders.listSnapshots(product.id)); } catch (e) { next(e); } });
  ...
  router.get('/rap/metadata', async (req, res, next) => { try { deprecate(res); res.json(await ancillary.rapMetadata(value(req.query.layer), value(req.query.timestamp))); } catch (e) { next(e); } });
  router.get('/rap/data', async (req, res, next) => { try { deprecate(res); send(res, await ancillary.rapData(value(req.query.layer), value(req.query.timestamp)), 'application/octet-stream'); } catch (e) { next(e); } });

...
export function notFound(req, res) {
  res.status(404).type('application/problem+json').json({ type: 'about:blank', title: 'Not Found', status: 404, detail: 'The requested resource does not exist.', instance: req.originalUrl, requestId: req.requestId });
}
...
export function errorHandler(error, req, res, next) { // eslint-disable-line no-unused-vars
  ...
  res.status(status).type('application/problem+json').json({ type: 'about:blank', title, status, detail: status >= 500 ? 'The requested artifact is temporarily unavailable.' : error.message, instance: req.originalUrl, requestId: req.requestId });
}

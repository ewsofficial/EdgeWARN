#!/usr/bin/env python3
"""Plot GOES ABI colormaps from src/EWMRS/colormaps.json.

Generates one PNG per GOES colormap plus a combined gallery image to
...
def _load_goes_colormaps(colormaps_path: Path) -> list[dict]:
    ...
    goes_colormaps = [
        ...
        if str(c.get("name", "")).startswith("GOES_ABI_") or str(c.get("name", "")) in {"GOES_IR", "GOES_RGB_Raw"}
    ]
    if not goes_colormaps:
        raise ValueError(f"No GOES colormaps found in {colormaps_path}")

    return sorted(goes_colormaps, key=lambda c: str(c.get("name", "")))
...
def _build_mpl_colormap(colormap_def: dict) -> tuple[LinearSegmentedColormap, float, float]:
    ...
    for threshold in thresholds:
        ...
        rgb = threshold["rgb"]
        if len(rgb) != 3:
            raise ValueError(f"Colormap {colormap_def.get('name')} has non-RGB threshold {threshold}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot GOES ABI colormaps for visual verification")
    parser.add_argument(
        "--colormaps-file",
    ...
    parser.add_argument(
        "--output-dir",
        default="goes_colormap_plots",
        help="Directory where individual colormap PNGs are written",
    )
    parser.add_argument(
        "--gallery",
        default="goes_colormaps_gallery.png",
    ...
    goes_colormaps = _load_goes_colormaps(colormaps_path)

    for colormap_def in goes_colormaps:
        out_name = f"{colormap_def['name']}.png"
        _plot_single(colormap_def, output_dir / out_name)

    _plot_gallery(goes_colormaps, gallery_path)

    print(f"Plotted {len(goes_colormaps)} GOES colormaps")

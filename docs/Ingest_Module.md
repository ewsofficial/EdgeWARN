...
*   **HTTPS Fallback**: In addition to S3 (via `s3_sync.py`), the system includes an HTTPS fallback (`https_client.py`) for MRMS data. This is used by the logic in `downloader.py` and the main scheduler (`run.py`) to fetch data directly from the NCEP HTTPS server if S3 is unavailable or stale.
